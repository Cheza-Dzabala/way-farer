...
const queries = {
  ...
  bookings: {
    insertBooking: 'INSERT INTO bookings(trip_id, user_id, number_of_seats, created_on) VALUES ($1, $2, $3, $4) RETURNING *',
  ...
  trips: {
    insertTrip: 'INSERT INTO trips(status, origin, destination, fare, seating_capacity, available_seats,trip_date, bus_license_number) VALUES ($1, $2, $3, $4, $5, $6,$7, $8) RETURNING *',
    ...
    modifySeats: 'UPDATE trips SET available_seats = $1  WHERE id = $2',
  },

  createTables: {
    bookingsTable: `CREATE TABLE IF NOT EXISTS bookings(
              id SERIAL, trip_id  INTEGER NOT NULL REFERENCES trips(id) ON DELETE CASCADE, user_id  INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, number_of_seats VARCHAR (128) NOT NULL, created_on VARCHAR (128) NOT NULL, PRIMARY KEY(trip_id, user_id)
            )`,
    tripsTable: `CREATE TABLE IF NOT EXISTS trips(
              id SERIAL PRIMARY KEY, bus_license_number VARCHAR(128) NOT NULL, origin VARCHAR(128) NOT NULL, destination VARCHAR(128) NOT NULL, fare FLOAT(2) NOT NULL, seating_capacity INTEGER NOT NULL, available_seats INTEGER NOT NULL, trip_date VARCHAR(128) NOT NULL, status BOOLEAN DEFAULT 'true'
            )`,
